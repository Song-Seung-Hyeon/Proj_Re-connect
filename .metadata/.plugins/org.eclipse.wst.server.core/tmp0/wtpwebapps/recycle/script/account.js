...
initialize()
  ...
  // 음주여부
var hasAlcoholRadio = document.getElementById("has_drinking");
var nonAlcoholRadio = document.getElementById("non_drinking");
var alcoholCountContainer = document.getElementById("alcohol_count_container");

hasAlcoholRadio.addEventListener("change", function () {
  // "베베있음" 선택 시 자녀수 선택 창 표시
  if (hasAlcoholRadio.checked) {
    alcoholCountContainer.style.display = "block";
  } else {
    alcoholCountContainer.style.display = "none";
  }
});

nonAlcoholRadio.addEventListener("change", function () {
  // "베베없음" 선택 시 자녀수 선택 창 숨김
  if (nonAlcoholRadio.checked) {
    alcoholCountContainer.style.display = "none";
  }
});
