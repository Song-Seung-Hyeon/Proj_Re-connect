...
<head>
<title>공지사항 | 마음을 다시 연결하는 결혼 정보 플랫폼 DASH</title>
<meta charset="UTF-8" />
...
<body>
	...
	try {
	    ...
	    int uid = Integer.parseInt(getUid); // 문자열로 받은 uid를 정수형으로 변환

		Class.forName("com.mysql.cj.jdbc.Driver");
		Connection conn = DriverManager.getConnection(url, user, password);
		
        String updateQuery = "UPDATE notice SET notice_views = notice_views + 1 WHERE uid = ?";
        PreparedStatement updateStatement = conn.prepareStatement(updateQuery);
        updateStatement.setInt(1, uid);
        updateStatement.executeUpdate();
		

		String selectQuery = "SELECT * FROM notice WHERE uid = ?";
	...
	<main>
		...
		<div class="board_wrap">
			<div class="board_title">
				<strong><a href="notice.jsp">공지사항</a></strong>
				<p>공지사항</p>
			</div>
			<table class="view">
				<thead>
					<tr>
						<th colspan="8" class="title"><%= resultSet.getString("notice_title") %></th>
					</tr>
				</thead>
				<tbody>
					<tr class="info">
						...
						<th>작성자</th>
						<td><%= resultSet.getString("notice_writer") %></td>
						<th>작성일</th>
						<td><%= formatNoticeDate %></td>
						<th>조회</th>
						<td><%= resultSet.getString("notice_views") %></td>
					</tr>
					<tr>
						<td colspan="8" class="content"><textarea id="textarea" oninput="autoResize()" readonly><%= resultSet.getString("notice_content") %></textarea></td>
					</tr>
										<tr>
						<%
						// 이미지 URL 가져오기
			...
			<div class="btn_box">
				<a href="notice.jsp" class="btn_back">목록</a>
			</div>
		</div>
	</main>
	<%
