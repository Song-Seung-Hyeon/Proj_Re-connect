fetch('../jsp/session.jsp')
	...
	.then(data => {
		...
		if (data.trim() === 'admin') {
			...
			loginStatusElement.innerHTML += '<div><a href="admin_manager_management.jsp">마이페이지</a></div>';
		} else if (data.trim() === 'manager') {
			// 매니저 권한인 경우
			loginStatusElement.innerHTML = '<div><a href="../jsp/logout.jsp">로그아웃</a></div>';
			loginStatusElement.innerHTML += '<div><a href="account_manager.jsp">마이페이지</a></div>';
